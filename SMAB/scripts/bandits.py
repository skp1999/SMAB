...
class Bandit:
    ...
    def __init__(self, K, N, hash_, gram_tuple,total_edges, high_reward=None,
...
#                 all_mlm_scores.append(mlm_score)
              edge_mlm_pred_labels.append(edge[2]) # 2 or 4
    ...
    def generate_local_sensitivity(self, index):
        ...
        if len(curr_word_edges) != 0:
            ...
            if (self.NRC_edgeweight == "yes") and self.PPL_edgeweight == "yes" :
                ...
                average_local_sensitivity = self.epsilon_greedy_binary_reward(curr_word_edges)
                print(f'Average Local Sensitivity: {average_local_sensitivity}')
#                 difference_reward = self.high_reward[curr_word][0] - average_local_sensitivity
